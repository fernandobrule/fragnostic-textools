...
trait Iso2TextServiceImpl extends Iso2TextServiceApi {

  def iso2TextService = new DefaultIso2TextService

  class DefaultIso2TextService extends Iso2TextServiceApi with FilesSupport with TextoolSupport {
      ...
      fileToList(pathSrc, charsetName) fold (
        ...
        list =>
          writeLinesToFile(list map (
            (line: String) => convert(line, iso2textMap.keysIterator, iso2textMap)), pathTgt) fold (
            error => Left("iso.2.text.error"),
